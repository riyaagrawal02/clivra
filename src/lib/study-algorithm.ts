...
export function calculateTopicPriority(
  ...
  const urgencyScore = daysUntilExam <= 7 
    ? 100 
    : daysUntilExam <= 30 
    ? 80 
    : daysUntilExam <= 60 
  ...
  if (topic.last_studied_at) {
    const daysSinceStudy = Math.floor(
      (Date.now() - new Date(topic.last_studied_at).getTime()) / (1000 * 60 * 60 * 24)
    );
    recencyScore = daysSinceStudy >= 14 
      ? 100 
      : daysSinceStudy >= 7 
      ? 75 
      : daysSinceStudy >= 3 
  ...
  const completionRate = topic.estimated_hours > 0 
...
export function calculateExamReadiness(
  ...
  const normalizedTime = daysUntilExam > 0 ? Math.min(daysUntilExam / 30, 1) : 0;
  ...
  if (percentage >= 80) {
    ...
    status = 'not_ready';
    message = "Let's build your study momentum. Start with high-priority topics.";
  }
...
export function rebalanceAfterMissed(
  ...
  const maxExtraPerDay = Math.round(dailyStudyMinutes * (maxOverloadPercentage / 100));
